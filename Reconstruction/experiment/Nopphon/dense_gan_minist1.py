...
S_train = S_train[:, select_voxel_idx] # dim_size = 2 ()
...
label_train_paired = np_utils.to_categorical(label_train_paired - 1, nb_classes)  # One hot label
...
def sample_SITL(S, I, T, L, size):
    start_idx = np.random.randint(0, S.shape[0] - size)  # (10,...)
...
#############################################################################
h2, h4, h5 = encoding_net_y2h(Y, False, training)  # h2 = fc2 (EEG latent), h4 = before softmax, h5 = (label) after softmax
...
h_paired = tf.nn.tanh(h2)  # h_paired is FMRI latent feed to generator
...
# i_t_mb
d_real = discriminator_net(image_input, h_paired, h5, False, training)  # paired stuff # IMG input, eeg_latent, eeg_label
...
d_real_unpaired = discriminator_net(X_unpaired_64, h_unpaired, net5_unpaired, True, training)  # In unpaired case, we use image latent
...
###################################################################################
theta_y = 0.5 * tf.matmul(tf.nn.tanh(h2), tf.transpose(tf.nn.tanh(net2)))  # Theta_y is sigma (NxN)
logloss_y = -tf.reduce_sum(tf.multiply(sim, theta_y) - log(1.0 + tf.exp(theta_y)))  # J1 (Inside reduce some will be multiple)
loss_y = tf.div(logloss_y, float(10 * 10))  # J1_hat

theta_x = 0.5 * tf.matmul(tf.nn.tanh(h2), tf.transpose(tf.nn.tanh(net2_unpaired)))
logloss_x = -tf.reduce_sum(tf.multiply(sim_u, theta_x) - log(1.0 + tf.exp(theta_x)))  # J4
loss_x = tf.div(logloss_x, float(10 * mb_size))  # J4_hat
...
classification_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=h4, labels=l)) + tf.reduce_mean(
    tf.nn.softmax_cross_entropy_with_logits(logits=net4, labels=l)) + tf.reduce_mean(
    tf.nn.softmax_cross_entropy_with_logits(logits=net4_unpaired, labels=l_u))
...
for idx in range(n_epochs):
    ...
    for it in range(0, num_train // mb_size):
        #
        class_step = 1  # L.shape = [10,...] # FMRI   # STIM       # ???    # Label             #??
        ...
        ll_mb = np.dot(l_t_mb, l_t_mb.T)  # <<<<< shape = [10, 10] GO!!! Sij is [NxN]
        ll_mbu = np.dot(l_t_mb, l_tu_mb.T)  # <<<<<< shape = [10, 64]
