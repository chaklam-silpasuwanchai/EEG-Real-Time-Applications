...
from tkinter.ttk import Label, Button, Frame, Style, Progressbar, Style
...
import time
import pyglet
pyglet.font.add_file("/home/chanapa/BCI_fork/BCI/P300/utils/char_generator/THSarabunNew/THSarabunNew.ttf")
...
class P300Window(object):
    def __init__(self, master: Tk, workQueue):
        ...
        self.imagesize = 110
        self.images_folder_path = '../../utils/images/th_images/'  #use utils/char_generator to generate any stimuli image
        self.flash_image_path = '../../utils/images/flash_images/chaky-ConvertImage.jpg'
        
        self.number_of_rows = 8
        self.number_of_columns = 9  # make sure you have 8 x 9 = 72 amount of images in the images_folder_path
        ...
        self.image_frame.grid(row=0, column=0, rowspan=8,
                              columnspan=8, padx=0, pady=0, ipadx=0, ipady=0)
        ...
        s = Style()
        s.theme_use('default')
        s.configure("black.Horizontal.TProgressbar", background='blue')
        self.progress_bar = Progressbar(self.master, length=1000, s='black.Horizontal.TProgressbar')
        self.progress_bar.grid(row = 10)
    ...
    def start_flashing(self):
        ...
        self.lsl_output.push_sample([1], local_clock())  # marker = 1 for event of flash  
        ...
        if((self.sequence_number + 1) % self.flash_per_target == 0):  #every flash_per_target, change letter 
            ...
            self.letter_idx += 1
            self.master.after(1, self.highlight_target)
        else:
            self.sequence_number = self.sequence_number + 1
            self.master.after(self.break_duration, self.start_flashing)
        
    def output_letter(self, receive): # output the predicted letter
    ...
    def get_predicted_result(self, ): # should input the classification result here
        print(f"=========== Getpredicted {self.letter_idx} ===========")
    ...
    def highlight_image(self, ):
        self.master.after(self.break_duration, self.unhighlight_image)

    def unhighlight_image(self):
